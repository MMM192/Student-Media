...
/**
 * Servlet implementation class Update_Student_profile
 */
@WebServlet("/Update_Student_profile")
public class Update_Student_profile extends HttpServlet {
...
HttpSession hs = request.getSession();
		...
		String StudentId = request.getParameter("student_id_update");
		...
		try {
			...
			PreparedStatement pst = con.prepareStatement(
				    "UPDATE add_student " +
				    "SET Sname = ?,  SAddress = ?, SMail = ?, SNumber = ?, Sinstitute_name = ?, SDeprtmentName = ?"
				    + ",  SDOB = ?, StudentId = ?, SUserid = ?, SPassword = ?" +
				    "WHERE Sid = ?");
			...
			pst.setString(4,Number_update);
			pst.setString(5,Institute_name_update);
			pst.setString(6,Department_name_update);
			...
			pst.setInt(11,(int) hs.getAttribute("Sid"));
		...
		response.sendRedirect("Student_Dashboard.jsp");
			...
			response.sendRedirect("Student_Dashboard.jsp?loo=pp");
